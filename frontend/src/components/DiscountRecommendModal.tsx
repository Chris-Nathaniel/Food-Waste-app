...
export default function DiscountRecommendModal({
  ...
  const fetchRecommendation = async () => {
    ...
    try {
      // ML API is served by Flask on port 5000 (see backend/ml_api.py)
      const response = await fetch("http://localhost:5000/api/ml/recommend-discount", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          // Expected by ml_api.py /api/ml/recommend-discount:
          // product, unit, quantity, price_IDR, expiry_days,
          // storage_temperature_C, day_of_week, is_weekend,
          // month, day, quarter, current_price
          product: product.name,
          unit: product.unit,
          quantity: product.quantity,
          price_IDR: product.unitPrice,
          ...
          // Calendar features are not stored in your Product model,
          // so we derive a best-effort from current date.
          day_of_week: (() => {
            const d = new Date();
            // JS: 0=Sun..6=Sat -> convert to 1-7 range expected by guide
      ...
      // Map ML API response into the shape expected by PricingRecommendation
      const suggestedDiscount = data.recommended_discount_percent ?? 0;
      const suggestedRetailPrice = Math.round(product.unitPrice * (1 - suggestedDiscount / 100));

      // We don't have rich text fields from the ML API, so we reuse the existing heuristic repurposing text
      ...
      const mapped = {
        ...
        reasoning: `ML memprediksi risk=${data.risk_level ?? "UNKNOWN"} dan demand=${data.predicted_demand ?? "?"}. Diskon direkomendasikan untuk mengurangi risiko kedaluwarsa dan meningkatkan perputaran stok.`,
        wasteImpactDescription: `Dengan menerapkan diskon terarah, ${product.name} lebih cepat tersalurkan sehingga mengurangi risiko pembuangan dan jejak karbon makanan terbuang.`,
      ...
      console.error("Local Optimizer Fallback Triggered: ", err);
      // // Construct local backup suggestion so the user never sees a broken modal or button
...
Menghubungi matriks ritel ML AI...
