    ...
    >
      ...
      {daysLeft === 0 && (
        <div className="bg-amber-100 px-4 py-1.5 border-b border-amber-200/50 flex justify-between items-center text-[10px] text-amber-950 font-extrabold tracking-wide uppercase">
          ...
          <span>Check shelf visuality</span>
        </div>
      )}
      ...
      <div className="p-5 flex-1 space-y-4">
        ...
        <div className="flex items-center justify-between bg-slate-50 border border-slate-150/50 rounded-xl p-3 text-xs">
          <span className="font-medium text-slate-500">Inventory Level:</span>
          <span className="font-bold text-slate-800 text-sm">
            {product.quantity > 0 ? (
              ...
              <span className="text-rose-500 font-extrabold italic uppercase">SOLD OUT</span>
            )}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 border-t border-slate-100 pt-3">
          <div className="space-y-0.5">
            <span className="text-[10px] font-bold text-slate-400 tracking-wider uppercase">Retail Unit Price</span>
          ...
          <div className="space-y-0.5 text-right">
            <span className="text-[10px] font-bold text-slate-400 tracking-wider uppercase">Projected Margin</span>
            <div>
              <span className={`inline-block text-xs font-extrabold px-1.5 py-0.5 rounded-md ${
                ...
                {profitMarginPercent > 0 ? `${profitMarginPercent}%` : "No Margin"}
              </span>
            </div>
          </div>
        </div>
        ...
        {showHistory && (
          ...
          >
            <span className="font-bold text-slate-400 uppercase tracking-widest text-[9px] block">History Ledger</span>
              ...
              {product.history?.map((hist, i) => (
                <div key={i} className="pl-4 relative">
                  ...
                  <div className="flex justify-between text-[10px] text-slate-400">
                    <span className="font-bold text-slate-700">{hist.action}</span>
      ...
      <div className="bg-slate-50/50 border-t border-slate-100 p-4 space-y-3">
        ...
        {product.quantity > 0 && daysLeft >= 0 && (
          <form onSubmit={handleSellCheckout} className="flex gap-1.5">
            ...
            >
              <ShoppingCart className="h-3.5 w-3.5" /> Sell Checkout
            </button>
          </form>
        )}
        ...
        <div className="flex flex-wrap items-center justify-between gap-2.5">
          ...
          >
            ...
            {showHistory ? "Collapse Log" : "Audit Timeline"}
          </button>
          ...
          <div className="flex items-center gap-1.5 ml-auto">
            ...
            {product.quantity > 0 && daysLeft >= 0 && (
              ...
              >
                <Sparkles className="h-3.5 w-3.5 animate-pulse text-amber-300" /> Optimized Price
              </button>
            )}
          </div>
        </div>
