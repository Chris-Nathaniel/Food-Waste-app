      ...
      case "Donated":
        return (
          <span className="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-800 bg-emerald-50 border border-emerald-100 px-2.5 py-1 rounded-full">
            <HandHeart className="h-3.5 w-3.5" /> Charity Donated
          </span>
        );
      case "Deli Repurposed":
        return (
          <span className="inline-flex items-center gap-1 text-[11px] font-bold text-indigo-800 bg-indigo-50 border border-indigo-100 px-2.5 py-1 rounded-full">
            <CookingPot className="h-3.5 w-3.5" /> Deli Kitchen
          </span>
        );
      case "Composted":
        return (
          <span className="inline-flex items-center gap-1 text-[11px] font-bold text-teal-800 bg-teal-50 border border-teal-100 px-2.5 py-1 rounded-full">
            <Leaf className="h-3.5 w-3.5" /> Composted
          </span>
        );
      case "Discarded":
        return (
          <span className="inline-flex items-center gap-1 text-[11px] font-bold text-rose-800 bg-rose-50 border border-rose-100 px-2.5 py-1 rounded-full">
            <Trash2 className="h-3.5 w-3.5" /> Discarded
          </span>
        );
    }
  };
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden text-slate-800 space-y-4 p-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h4 className="font-bold text-slate-900 text-base leading-tight">Mitigation Action Audit History</h4>
          <p className="text-xs text-slate-400 font-medium">Record logs of food waste write-offs, charity donations, and kitchen material repurposing.</p>
        </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Filter logs by product name or SKU..."
        ...
        <table className="w-full text-left border-collapse text-xs">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100 font-semibold text-slate-500 uppercase tracking-wider text-[10px]">
              ...
              <th className="py-3 px-4 text-center">Qty Logged</th>
              <th className="py-3 px-4 text-right">Written-Off Loss</th>
              <th className="py-3 px-4 text-right">Value Salvaged</th>
              <th className="py-3 px-4">Mitigation Move</th>
              <th className="py-3 px-4">Action Comments</th>
            </tr>
          </thead>
            ...
            {filteredLogs.length === 0 ? (
              <tr>
                <td colSpan={8} className="text-center py-10 text-slate-400 h-24">
                  No matching write-off logs found under chosen constraints.
                </td>
              </tr>
            ) : (
                ...
                >
                  ...
                  <td className="py-3 px-4 whitespace-nowrap text-slate-600 font-semibold">{log.category}</td>
